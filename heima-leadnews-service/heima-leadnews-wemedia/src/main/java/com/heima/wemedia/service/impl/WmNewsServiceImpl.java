...
public class WmNewsServiceImpl extends ServiceImpl<WmNewsMapper, WmNews> implements WmNewsService {
    ...
    /**
     * 功能一。如果封面为自动，则设置封面类型的数据
     * 匹配规则：
     * 1。1<=内容图片数<3,单图 type =1
     * 2.内容图片数 >=3,多图 type =3
     * 3.无图 type = 0
     *
     * 功能二，保存封面图片和素材的关系
    ...
    public ResponseResult downOrUp(WmNewsDto dto) {
        ...
        if(!wmNews.getStatus().equals(WmNews.Status.PUBLISHED.getCode())){
            return ResponseResult.errorResult(AppHttpCodeEnum.PARAM_INVALID,"当前文章未发布，不能上架！");
        }
        //4.修改文章enable 默认审核通过直接上架
        if(dto.getEnable()== null || dto.getEnable() <=-1 || dto.getEnable() >= 2){
            return ResponseResult.errorResult(AppHttpCodeEnum.PARAM_INVALID,"文章发布状态异常！");
        }
        update(Wrappers.<WmNews>lambdaUpdate().set(WmNews::getEnable,dto.getEnable()).eq(WmNews::getId,dto.getId()));
        //查询文章id是否存在，也就是文章是否审核通过
        if(wmNews.getArticleId()!=null){
            //发送消息通知artcile，修改文章配置
